...
class DPOTrainer(ABC):
    ...
    def __init__(
        ...
        self.average_log_prob = self.args.average_log_prob
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_logps, rejected_logps, aux_loss, nll_loss = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens, average_log_prob=self.average_log_prob
                    )
                    with torch.no_grad():
                        reference_chosen_logps, reference_rejected_logps, _, _ = self.concatenated_forward(
                            self.ref_model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens, average_log_prob=self.average_log_prob
                        )
                else:
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                logprob_acc_sum += (chosen_logps > rejected_logps).float().mean().item()
                loss_sum += loss.item()
                times += 1
                step_bar.update()
            ...
            logs = {
                ...
                "logprob_acc_mean": logprob_acc_sum / times
            }
            ...
            if self.strategy.is_rank_0():
                ...
                elif getattr(self.args, "results_path", None):
                    import json
                    os.makedirs(os.path.dirname(self.args.results_path), exist_ok=True)
                    with open(self.args.results_path, "a") as f:
                        f.write(json.dumps(logs) + "\n")
        self.model.train()  # reset model state

    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens, average_log_prob=False):
