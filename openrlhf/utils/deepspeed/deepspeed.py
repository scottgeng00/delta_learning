...
class DeepspeedStrategy(ABC):
    ...
    def check_if_finished(self, output_dir):
        return os.path.exists(os.path.join(output_dir, FINISHED_FILE_TAG))
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            # corner case for tie_word_embeddings, such as Qwen2-0.5B or Llama-3.2-3B
            if getattr(model_to_save.config, "tie_word_embeddings", False):
                if "lm_head.weight" in state_dict_keys:
                    state_dict_keys.remove("lm_head.weight")
                if isinstance(model_to_save, PeftModel) and "base_model.model.lm_head.weight" in state_dict_keys:
                    state_dict_keys.remove("base_model.model.lm_head.weight")
            ...
            if kwargs.get("cleanup_checkpoints", False) and kwargs.get("ckpt_dir", None):
                if os.path.exists(kwargs["ckpt_dir"]):
                    shutil.rmtree(kwargs["ckpt_dir"])
            if kwargs.get("write_finished", False):
                with open(os.path.join(output_dir, FINISHED_FILE_TAG), "w") as f:
                    f.write("")
    ...
    def save_ckpt(self, model, save_dir, tag=None, max_num=3, max_mem=1000, client_state={}, save_latest=True):
        ...
        if self.is_rank_0():
            ...
            # we should delete the oldest ckpt if the number of ckpts exceeds max_num or the total size exceeds MAX_SIZE, except the latest one
            while True:
                subdirs = sorted(
                    [
                ...
                if len(subdirs) > max_num or (total_size > MAX_SIZE and len(subdirs) > 1):
